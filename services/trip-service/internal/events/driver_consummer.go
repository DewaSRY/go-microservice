...
func (c *driverConsumer) handleTripAccepted(ctx context.Context, tripID string, driver *pbd.Driver) error {
	...
	if trip == nil {
		return fmt.Errorf("trip_was_not_found %s", tripID)
	}
	...
	if err := c.rabbitmq.PublishingMessage(ctx, contracts.TripEventDriverAssigned, contracts.AmqpMessage{
		OwnerID: trip.UserId,
		Data:    marshalledTrip,
	}); err != nil {
		return err
...
func (c *driverConsumer) handleTripDeclined(ctx context.Context, tripID, riderID string) error {
	...
	if err := c.rabbitmq.PublishingMessage(ctx, contracts.TripEventDriverNotInterested,
		contracts.AmqpMessage{
			OwnerID: riderID,
			Data:    marshalledPayload,
		},
